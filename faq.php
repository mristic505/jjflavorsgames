<style type="text/css">
	.logo {
		display: none;
	}
</style>
<div class="faqContainer">
	
	<div class="flavorLogoImg">
		<img src="img/jj-flavor-logo.png">
	</div>
	
	<h3>Who is eligible to play the Juicy Juice Flavor Discovery Instant Win Game?</h3>
	<p>The Instant Win Game is open to all legal residents of the 50 United States (including D.C.) who are 18 years of age or older or the age of majority in their state of residence, whichever is greater.  Entrants must have internet access and a valid e-mail address to participate.</p>

	<h3>How do I play?</h3>
	<p>Visit <a href="#">www.juicyjuice.com/flavordiscovery</a> beginning at 5:00 p.m. ET on February 5, 2018, through 11:59:59 p.m. ET on April 30, 2018 (the “Game Period”), and follow the on-screen instructions to spin the Game wheel to see if you are a potential winner. <a href="#">Data rates may apply to Game play on a mobile device.</a></p>

	<h3>If I land on the “winner” space, am I a winner?</h3>
	<p>Players who land on the “winner” space are potential winners until eligibility and compliance with the Official Rules have been verified.</p>

	<h3>What if I land on one of the sections of the wheel other than “winner”?</h3>
	<p>If you land on any other spaces on the game wheel, you will be invited to play a game, work a puzzle or complete an activity. Activities are for amusement only and have no impact on winner determination or prize award.</p>

	<h3>What is the prize?</h3>
	<p>The prize is a $40 prepaid card. There are a total of 85 prizes available to be won.</p>

	<h3>How is the winner determined?</h3>
	<p>Prizes are randomly time-seeded throughout the Game Period. A potential winner will be the first eligible person to play at or after the randomly selected time.</p>

	<h3>Can I win more than once?</h3>
	<p>There is a limit of one (1) prize per person/household.</p>

	<h3>If I win, how long will it take to get my prize?</h3>
	<p>Please allow 4–6 weeks for delivery of prizes following winner verification. All prizes will be delivered by mail to the mailing address provided by the winner on the prize claim form.</p>

</div>
